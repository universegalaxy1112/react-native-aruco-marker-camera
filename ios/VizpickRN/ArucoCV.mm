...
+(NSMutableArray *) estimatePose:(CVPixelBufferRef)pixelBuffer withIntrinsics:(matrix_float3x3)intrinsics andMarkerSize:(Float64)markerSize corners:(NSArray**)outCorners ids:(NSArray**)outIds {
    ...
    for(int i = 0; i < ids.size(); i++) {
      [idArray addObject: [NSNumber numberWithInt:ids[i]]];
    }
  
    for (int i = 0; i < corners.size(); i++) {
      std::vector<cv::Point2f> value = corners[i];
      NSMutableArray *valueArray = [[NSMutableArray alloc] init];
      for (int j = 0; j < value.size(); j++) {
        cv::Point2f point = value[j];
        float x = point.x;
        float y = point.y;
        [valueArray addObject:@[[NSNumber numberWithFloat: x], [NSNumber numberWithFloat: y]]];
      }
      [cornerArray addObject:valueArray];
    }
    ...
    *outIds = idArray;
    *outCorners = cornerArray;
    
    return arrayMatrix;
}

+(NSArray *) estimatePose:(CVPixelBufferRef)pixelBuffer andMarkerSize:(Float64)markerSize {
    
    std::vector<int> ids;
    std::vector<std::vector<cv::Point2f>> corners;
    detect(corners, ids, pixelBuffer);
    
    NSLog(@"Corners: %lu", corners.size());
    NSLog(@"Ids: %lu", ids.size());
    
    NSMutableArray *cornerArray = [NSMutableArray new];
    ...
    for(int i = 0; i < corners.size(); i++) {
        ...
        for (int j = 0; j < corners[j].size(); j++) {
            ...
            [points addObject:[NSNumber numberWithFloat:point.y]];
            [pointArray addObject:points];
        }
        
        [cornerArray addObject:pointArray];
    }
    
    for(int i = 0; i < ids.size(); i++) {
        [idsArray addObject:[NSNumber numberWithInt:ids[i]]];
    }
    
    return @[cornerArray, idsArray];
}
